...
// @LINE:113
case controllers_TagController_getAllTagWeights79_route(params) => {
   call { 
        controllers_TagController_getAllTagWeights79_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.TagController]).getAllTagWeights)
   }
}
        

// @LINE:118
case controllers_Assets_at80_route(params) => {
   call(Param[String]("path", Right("/public")), params.fromPath[String]("file", None)) { (path, file) =>
        controllers_Assets_at80_invoker.call(controllers.Assets.at(path, file))
   }
}
